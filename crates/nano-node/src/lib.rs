...
use nano_burnchain::{BurnBlock, BurnchainSource};
...
pub fn replay_one<S: BurnchainSource>(source: &S, height: u64) -> Result<(), ReplayFailure> {
    let block = source
        .block_at(height)
        .map_err(|_| ReplayFailure::BurnchainInput)?;
...
impl BurnchainSource for BaselineSource {
