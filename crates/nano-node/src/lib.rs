    ...
    S::Error: fmt::Display,
{
    ...
    pub async fn catch_up(
        ...
        let executed = self.execute_staged(node, pox, staging, budget.execute).await?;
    ...
    async fn context_for(
        ...
        if let Some(view) = block.bitcoin_view_consensus_hash() {
            ...
            let Some(walked) = ended_by_a_rate_limit(Self::bitcoin_view_of(node, block).await)?
            else {
                return Ok(None);
            };
        ...
        let Some(sortition) = ended_by_a_rate_limit(self.sortition_for(node, view).await)? else {
            return Ok(None);
        };
    ...
    async fn execute_staged(
        ...
        while executed < budget {
            ...
            let Some((bitcoin_context, sortition)) = self
                .context_for(node, pox, &block, &mut timing, &mut previous_view)
                .await?
            else {
                rate_limited = true;
                break;
