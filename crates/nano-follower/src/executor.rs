...
impl LocalSortition {
    ...
    /// it.
    #[must_use]
    pub const fn from_snapshot(snapshot: &nano_sortition::SortitionSnapshot) -> Self {
        Self {
    ...
    pub fn record(self, bitcoin_context: &mut BitcoinBlockContext) {
    ...
    pub const fn record_authentication(self, context: &mut BitcoinBlockContext) {
...
#[cfg(all(test, feature = "node-adapters"))]
    ...
    S::Error: fmt::Display,
{
    ...
    pub fn into_validator_parts(self) -> (ChainState, NakamotoBlock, u64, S) {
    ...
    /// Drain ancestor header identifiers Clarity requested but this state lacks.
    pub fn take_missing_headers(&mut self) -> Vec<[u8; 32]> {
        self.chainstate.take_missing_headers()
    }

    /// Whether one historical block header is already available to Clarity.
    pub fn knows_block_header(&self, block: &[u8; 32]) -> bool {
        self.chainstate.knows_block_header(block)
    }

    /// Add one independently placed ancestor header to the local execution view.
    pub fn backfill_ancestor_header(
        ...
        self.chainstate.backfill_ancestor_header(
            block,
            burn_hash,
            burn_height,
            timestamp,
    ...
    pub fn local_burn_tip(&self) -> Result<nano_sync::SortitionInfo, CheckpointExecutionError> {
        let tracker = self.sortition.as_ref().ok_or_else(|| {
    ...
    pub fn local_executed_burn_view(
        &self,
    ...
    pub fn registered_local_miner_keys(&self) -> Vec<nano_primitives::Hash160> {
    ...
    pub fn local_miner_slot_writers(&self) -> Option<[nano_primitives::Hash160; 2]> {
        self.sortition
            .as_ref()
    ...
    pub fn follow_burnchain_deferred(
        &mut self,
        pox: &PoxInfo,
    ...
    #[cfg(feature = "node-adapters")]
    pub fn announce_burn_blocks(&self, notifications: &[sortition::BurnNotification]) {
        let Some(observers) = self.observers.as_ref() else {
            return;
        };
    ...
    pub fn local_ancestor_burn_context(
        &self,
