...
def is_test_attribute(text: str) -> bool:
    ...
    if not compact.startswith("#[") or not compact.endswith("]"):
        return False
    attribute = compact[2:-1]
    macro = attribute.split("(", 1)[0].rsplit("::", 1)[-1]
    if macro in {"test", "bench", "rstest"}:
        return True
    if not attribute.startswith("cfg(") or not attribute.endswith(")"):
        return False
...
def line_counts(
    source: bytes, test_ranges, comment_ranges, whole_file_test: bool
) -> tuple[int, int]:
