...
impl Word for Ed25519Verify {
    fn name(&self) -> ClarityName {
        ClarityName::from_literal("ed25519-verify")
    }
}

impl ComplexWord for Ed25519Verify {
    fn traverse(
        &self,
        generator: &mut WasmGenerator,
        builder: &mut walrus::InstrSeqBuilder,
...
impl ComplexWord for Decompress {
    fn traverse(
        ...
        let return_type = generator
            .get_expr_type(expr)
            .ok_or_else(|| GeneratorError::TypeError("secp256k1-decompress? must be typed".into()))?
            .clone();
        let (result, size) = generator.create_call_stack_local(builder, &return_type, true, true);
        builder.local_get(result).i32_const(size);
        builder.call(generator.func_by_name("stdlib.secp256k1_decompress"));
        generator
