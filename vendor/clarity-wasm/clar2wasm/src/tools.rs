...
use clarity::vm::contexts::{EventBatch, GlobalContext, OwnedEnvironment};
...
use clarity::vm::{
    eval_all, ClarityVersion, ContractContext, ContractName, SymbolicExpression, Value,
};
...
impl TestEnvironment {
    ...
    /// The interpreter's counterpart to [`Self::call_contract`], so a public
    /// function can be charged through both runtimes from the same state.
    pub fn interpret_call_contract(
        &mut self,
        contract_name: &str,
        function_name: &str,
        arguments: &[Value],
        ...
        let (_, tracker) = environment
            .destruct()
            .expect("interpreted call should leave the database usable");
        self.cost_tracker = tracker;
        result.map(|(value, _, events)| {
            self.events.push(EventBatch { events });
            value
        })
    }
...
/// Assert that the compiler and the interpreter charge the same five cost
/// dimensions for calling `function` on a contract built from `snippet`.
///
/// Only the call is charged: the deployment runs on both sides first and its
