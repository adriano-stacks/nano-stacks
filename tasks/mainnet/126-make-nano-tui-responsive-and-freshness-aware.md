...
- [ ] Parse and validate `--rpc-url`, optional `--metrics-url` and `--once` with
      a conventional CLI parser; document the binary and exit behavior.
- [ ] Move HTTP polling and block backfill out of the render/input loop.
- [ ] Fetch independent status routes concurrently and deliver partial snapshots.
- [ ] Track loading, fresh, stale and unavailable state per RPC source, including
      last-success time and the latest short error.
- [ ] Backfill blocks incrementally without delaying keys, redraw or shutdown.
- [ ] Add wide, standard 80x24 and explicit too-small layouts.
- [ ] Cover slow, timed-out, partial and long-value snapshots at 80x24, 110x32
      and a wide terminal.
